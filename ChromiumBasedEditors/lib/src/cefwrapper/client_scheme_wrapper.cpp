...
public:
								...
								CefRefPtr<CefCallback> callback) OVERRIDE
	{
		...
		std::string url = request->GetURL().ToString();
		std::string::size_type posFind = url.find("univaultoffice://plugin/");
		if (posFind != std::string::npos)
		{
...
namespace asc_scheme
{
							   ...
							   )
	{
		...
		registrar->AddCustomScheme("univaultoffice", CEF_SCHEME_OPTION_STANDARD | CEF_SCHEME_OPTION_SECURE | CEF_SCHEME_OPTION_CORS_ENABLED | CEF_SCHEME_OPTION_FETCH_ENABLED);
	}
#endif
	...
	bool InitScheme(CAscApplicationManager* pManager)
	{
		...
		res = res && CefRegisterSchemeHandlerFactory("univaultoffice", "", new ClientSchemeHandlerFactory(pManager));
		return res;
	}
